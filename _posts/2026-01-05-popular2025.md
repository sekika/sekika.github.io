...
総ページビュー数の年変化をグラフにしてみました。2025年のアクセスが少ないのは、今年度は国内研究で授業をしていないため、授業の履修者からのアクセスがないことが影響をしているかもしれません。2023年にアクセスが増えているのは、[Kaidoku](https://sekika.github.io/kaidoku/)等のサイトにGoogleアナリティクスを入れたためです。
...
const layout = {
    title: { text: 'Annual page view' },
    xaxis: { title: { text: 'Year' } },
    yaxis: { title: { text: 'Access' }, range: [0, Math.max(...access) * 1.1] }
};
